...
export default function QuoteItemActions({ quote, rtl }: Props) {
  ...
  return (
    <>
      ...
      {confirmOpen ? (
        <div className="fixed inset-0 z-[76]" role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-black/60" onClick={() => (!deleting ? setConfirmOpen(false) : null)} />
          <div
            dir={rtl ? "rtl" : "ltr"}
          ...
          >
            ...
            <div className="mt-2 text-sm text-zinc-600 dark:text-zinc-300">
              {rtl ? "هل تريد حذف هذا الاقتباس؟ لا يمكن التراجع." : "Delete this quote? This action cannot be undone."}
            </div>
            ...
            <form onSubmit={onSave} className="mt-4 grid gap-3" dir={rtl ? "rtl" : "ltr"}>
              <div>
                ...
                {dirty.name && !nameValid ? (
                  <p className="mt-1 text-sm text-red-600">{rtl ? "الاسم يجب أن يكون 2–80 حرف" : "Name must be 2–80 characters"}</p>
                ) : null}
              </div>
              ...
              <div>
                ...
                {dirty.book && !bookValid ? (
                  <p className="mt-1 text-sm text-red-600">{rtl ? "اسم الكتاب يجب أن يكون 2–160 حرف" : "Book title must be 2–160 characters"}</p>
                ) : null}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className={"w-full min-h-28 rounded-md border border-zinc-200/60 dark:border-zinc-800/60 bg-background px-3 py-2 " + (rtl ? "text-right" : "text-left")}
                />
                {dirty.text && !textValid ? (
                  <p className="mt-1 text-sm text-red-600">{rtl ? "الاقتباس يجب أن يكون 3–1200 حرف" : "Quote must be 3–1200 characters"}</p>
                ) : null}
              </div>
